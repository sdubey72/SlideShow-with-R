...
\hlstd{drac_pos}\hlkwb{<-}\hlstd{drac_words}\hlopt{%>%}
  ...
  \hlkwd{top_n}\hlstd{(}\hlnum{8}\hlstd{,}\hlkwc{wt}\hlstd{=count)}
...
\hlstd{drac_pos[}\hlnum{4}\hlopt{:}\hlnum{6}\hlstd{,]}
\end{alltt}
\begin{verbatim}
## # A tibble: 3 x 3
...
\hlstd{drac_neg}\hlkwb{<-}\hlstd{drac_words}\hlopt{%>%}
  ...
  \hlkwd{top_n}\hlstd{(}\hlnum{8}\hlstd{,}\hlkwc{wt}\hlstd{=count)}
\hlstd{drac_neg}\hlopt{$}\hlstd{word}\hlkwb{<-}\hlkwd{factor}\hlstd{(drac_neg}\hlopt{$}\hlstd{word,}\hlkwc{level}\hlstd{=drac_neg}\hlopt{$}\hlstd{word)}
\hlstd{drac_neg[}\hlnum{1}\hlopt{:}\hlnum{4}\hlstd{,]}
\end{alltt}
\begin{verbatim}
## # A tibble: 4 x 3
##      word count sentiment
